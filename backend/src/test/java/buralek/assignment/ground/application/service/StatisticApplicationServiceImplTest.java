...
class StatisticApplicationServiceImplTest {
    ...
    @Test
    @DisplayName("WHEN locationId is null, THEN statisticDomainService.calculate() is called with null locationId")
    void getStatisticSample1() {
        stubThresholdProperties();
        when(statisticDomainService.calculate(eq(null), any())).thenReturn(buildStatistics());

        statisticApplicationService.getStatisticSample(null);

        verify(statisticDomainService).calculate(eq(null), any());
    }

    @Test
    @DisplayName("WHEN locationId is provided, THEN statisticDomainService.calculate() is called with that locationId")
    void getStatisticSample2() {
        stubThresholdProperties();
        when(statisticDomainService.calculate(eq(LOCATION_ID), any())).thenReturn(buildStatistics());

        statisticApplicationService.getStatisticSample(LOCATION_ID);

        verify(statisticDomainService).calculate(eq(LOCATION_ID), any());
    }
